    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        if(args.userRole == "Teacher")
        {
            ...
            binding.userLoginRegisterClickableTv.setOnClickListener {
                Toast.makeText(context,"Clicked on Register",Toast.LENGTH_SHORT).show()
            ...
            binding.userLoginLoginBtn.setOnClickListener {
                ...
                email = binding.userLoginEmailEt.text.toString()
                password = binding.userLoginPasswordEt.text.toString()
                Log.d("UserLoginFragment","Reached elseif block")
                verifyStudent()
...
//                }
            ...
            binding.userLoginRegisterClickableTv.setOnClickListener {
                Log.d("UserLoginFragment","Clicked on Register")
...
//    }
    fun verifyStudent(){
    Log.d("UserLoginFragment","Reached verify student function")
        var task2 = Task2UserLoginFragment()
        task2.execute()
    }
    ...
    inner class Task2UserLoginFragment : AsyncTask<Void, Void, Void>() {
        ...
        override fun doInBackground(vararg p0: Void?): Void? {
            try{
                ...
                Log.d("Test","OK!")
                var statement : Statement = conn.createStatement()
                var resultSet : ResultSet = statement.executeQuery("select st_Id from students where st_Email = '$email' and st_Password = '$password'")
                while (resultSet.next()){
                    currentStudentID += resultSet.getString(1)
                }
            }catch (e: Exception){
                error = e.toString()
            }
        ...
        override fun onPostExecute(result: Void?) {
            if(error != ""){
                ...
                if(currentStudentID!=""){
                    Log.d("Test","Email ID verified successfully")
                    Log.d("Test","Current student ID is: $currentStudentID")
                    val action = UserLoginFragmentDirections.actionUserLoginFragmentToStudentMainScreenFragment(currentStudentID)
                    findNavController().navigate(action)
                }else{
                    Toast.makeText(context,"Email ID doesn't exist!",Toast.LENGTH_SHORT).show()
                    Log.d("Test","Email ID doesn't exist!")
